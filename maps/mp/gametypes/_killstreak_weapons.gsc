...
init()
{
	...
	//maps\mp\gametypes\_hardpoints::registerKillstreak("mp40_drop_mp", "mp40_drop_mp", "killstreak_mp40_drop", "mp40_used", ::useCarriedKillstreakWeapon, false, false, "MP40_USED" );
	//maps\mp\gametypes\_hardpoints::registerKillstreakAltWeapon( "mp40_drop_mp", "mp40_blinged_mp" );
	//maps\mp\gametypes\_hardpoints::registerKillstreakStrings("mp40_drop_mp", &"KILLSTREAK_EARNED_MP40", &"KILLSTREAK_MP40_NOT_AVAILABLE", &"KILLSTREAK_MP40_INBOUND" );
	//maps\mp\gametypes\_hardpoints::registerKillstreakDialog("mp40_drop_mp", "mpl_killstreak_mp40", "kls_mp40_used", "","kls_mp40_enemy", "", "kls_mp40_ready");
	//maps\mp\gametypes\_hardpoints::registerKillstreakDevDvar("mp40_drop_mp", "scr_givemp40");
...
useCarriedKillstreakWeapon( hardpointType )
{
	...
	/*currentKillstreakId = maps\mp\gametypes\_hardpoints::getTopKillstreakUniqueId();
...
checkIfSwitchableWeapon( currentWeapon, newWeapon, killstreakWeapon, currentKillstreakId )
{
	...
	if ( self HasWeapon( killstreakWeapon ) && !self GetAmmoCount( killstreakWeapon ) )//Safty check that we're holding an empty killstreak weapon
		...
		switchableWeapon = true;
	else if( maps\mp\gametypes\_hardpoints::isKillstreakWeapon( newWeapon ) )//allow killstreaks to be called in
		switchableWeapon = false;
